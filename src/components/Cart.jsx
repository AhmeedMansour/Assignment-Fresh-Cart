...
export default function Cart() {
  ...
  const [hasFetched, setHasFetched] = useState(false); // Track if data has been fetched once

  // Use query hook with `enabled` option to trigger data fetching only once
  const { data, isLoading, isFetching, refetch } = useQueryCart(getCarts, {
    enabled: !hasFetched, // Disable fetching until initial render
  });

  // Ensure state is updated after the first fetch
  useEffect(() => {
    if (data && !hasFetched) {
      setCartNum(data.data.numOfCartItems);
      setHasFetched(true); // Set flag to avoid further fetches
    }
  }, [data, setCartNum, hasFetched]);

  const { mutate, isPending } = useMutationCart(deleteItem);
  const { mutate: mutateClear, isPending: isPendingClear } = useMutationCart(clearCart);
  const { mutate: mutateUpdate, isPending: isPendingUpdate } = useMutationCart(updateCount);
  ...
  const handleUpdateCount = useCallback((productId, count) => {
    mutateUpdate({ productId, count });
  }, [mutateUpdate]);

  // Ensure component doesn't re-fetch the data automatically
  useEffect(() => {
    if (data && !hasFetched) {
      setCartNum(data.data.numOfCartItems);
      setHasFetched(true); // Mark as fetched
    }
  }, [data, hasFetched, setCartNum]);
  ...
  // Show loading screen if any mutation or query is still in progress
  ...
  return (
    <div className="w-3/4 mx-auto my-5 relative overflow-x-auto sm:rounded-lg">
      ...
      <h1 className="font-bold my-4">
        Total Price: <span className="text-green-color font-bold">{data?.data?.data?.totalCartPrice}</span> EGP
      </h1>
            ...
            <tr key={prod?.product?._id} className="bg-white border-b border-gray-200 hover:bg-gray-50 ">
              ...
              <td className="px-6 py-4">
                <div className="flex items-center">
                  <button onClick={() => handleUpdateCount(prod?.product?._id, prod?.count - 1)} 
              ...
              <td>
                <a onClick={() => handleRemoveItem(prod?.product?._id)} 
                   href="#" 
                   className="font-medium text-white bg-red-400 p-3 rounded dark:text-red-500 hover:underline hover:text-white cursor-pointer">
      ...
      <button onClick={handleClearCart} className="bg-green-color p-4 my-10 text-white rounded-lg cursor-pointer float-right">Clear Cart</button>
      <br />
